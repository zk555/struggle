...
//			return R.error(1, "演示系统不允许修改,完整体验请部署程序");
		...
		String fileName = struggleConfig.getUploadPath() + sysFileService.get(id).getUrl().replace("/files/", "");
...
//		FileDO sysFile = new FileDO(FileType.fileType(fileName), "/files/" + fileName, new Date());
		FileDO sysFile = new FileDO(FileType.fileType(fileName),"/files/"+fileName,new Date());
		try {
			FileUtil.uploadFile(file.getBytes(), struggleConfig.getUploadPath(), fileName);
		} catch (Exception e) {
			return R.error();
		}
